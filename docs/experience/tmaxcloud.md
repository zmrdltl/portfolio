...
Java/TypeScript 기반 No-code 플랫폼에서 UI 정의로 생성된 Java 서비스 코드와 SQL/DDL을 배포 전에 검증하는 기능을 개발했습니다. 데이터 변경 이력 저장을 구현하고, 과거 시점 조회에 필요한 유효 이력 행 선택 기준을 정리했습니다. SQL/DDL Generator와 Entity Export/Import의 일부 기능도 추가했습니다.
...
**구현:** WebSocket URL과 연결 상태 검사, 서비스 목록 조회, 항목별 JSON 요청 양식 생성, Monaco Editor 요청 편집, API 호출, 응답·DB 반영 확인 기능을 구현했습니다.
