...
- [ClumL](cluml.md): moved the request-limiter capacity check and state update into one lock scope to fix a race that had allowed requests to pass at more than 10x the effective limit, then externalized a repeated detection setting and reduced one setting-change workflow by more than 30%.
