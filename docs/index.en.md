...
- [Rust service improvements in a security analysis product](experience/cluml.md): moved the request-limiter capacity check and limit-state update into one lock scope, fixing a race that had allowed requests to pass at more than 10x the effective limit. I also externalized a repeatedly adjusted detection setting, reducing the work for one setting change by more than 30%.
