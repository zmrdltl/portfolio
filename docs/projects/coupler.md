...
**제약과 선택:** 기존 React Native 앱, Express API, React 관리자 웹, MySQL 데이터와 마이그레이션을 함께 바꿔야 했습니다. 초기 입력 부담을 줄이되 첫 심사에 필요한 기본정보와 필수 프로필은 유지하고, 승인 뒤 준회원·정회원 심사는 서로 독립적으로 진행할 수 있어야 했습니다. 클라이언트별 조건문을 맞추는 대신 API가 접근 상태와 다음 행동을 반환하는 단일 기준이 되고, 앱과 관리자 웹은 유효한 서버 상태만 해석하도록 선택했습니다. 상태가 없거나 유효하지 않으면 화면을 임의로 열지 않는 방향으로 처리했습니다.
...
**검증과 결과:** 행사 공개·확정·재개방·종료와 신청·승인·나가기·재신청·후기의 상태 전이, API·관리자 웹·모바일 회귀 테스트를 릴리스 기준에 포함했습니다. 채팅은 현재 설정된 행사 시작일 전날 13:00부터 열리고 행사 시작 시각 24시간 뒤부터 읽기 전용으로 전환됩니다. 이 생명주기는 [그룹미팅 시스템 문서](https://coupler-developer.github.io/docs/architecture/group-meeting-system/)로 고정하고 v2.3.0 범위로 출시했습니다. 출시 뒤 API 계약 전환 위반이 확인됐으며, FCM·실시간 연결·scheduler를 함께 확인하는 운영 smoke는 추가 검증 범위로 남았습니다.
