...
**검증과 결과:** 행사 공개·확정·재개방·종료와 신청·승인·나가기·재신청·후기의 상태 전이, API·관리자 웹·모바일 회귀 테스트를 릴리스 기준에 포함했습니다. 채팅은 현재 설정된 행사 시작일 전날 13:00부터 열리고 행사 시작 시각 24시간 뒤부터 읽기 전용으로 전환됩니다. 이 생명주기는 [그룹미팅 시스템 문서](https://coupler-developer.github.io/docs/architecture/group-meeting-system/)로 고정하고 v2.3.0 범위로 출시했습니다.
